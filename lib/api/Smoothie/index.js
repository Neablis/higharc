...
const SmoothieRouter = (0, _express.Router)(); // Exposing the smoothie Id for any down stream routers
...
SmoothieRouter.route("/").get(async (req, resp) => {
  ...
  const smoothies = await _Smoothie.default.find({
    where: {
      user: userId
    },
...
SmoothieRouter.route("/:id").get(async (req, resp) => {
  const {
    userId
  } = req.context;
  const {
